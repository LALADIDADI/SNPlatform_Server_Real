#include <iostream>		// 输入输出流文件 cin cout等
#include <vector>	// vector容器类
#include <stdlib.h>	// 标准库头文件，定义了五种类型，一些宏和通用工具函数 ,malloc()等
#include <stdio.h>	 // 标准输入输出文件, printf() scanf()等
#include <math.h>	// 一些数学相关函数，log(),sin()等
#include <time.h>	// 时间和日期头文件，时间和日期相关函数
#include <string.h>	// 字符串操作
#include <ctype.h>	// C语言字符分类函数，用于测试字符是否属于特定的字符类别,前缀c意为适应C++版本
#include <ctime>	// 把日期和时间转换为字符串
#include <cstdlib>	// 同stdlib的适应C++版本
#include <fstream>	// 文件的输入输出流
#include <sstream>	// StringStream，用于格式转换
#include <iomanip>	// 输入输出操作相关函数
#include <algorithm>	// 基于迭代器的序列操作函数 
#include <gsl/gsl_cdf.h>	// GNU Scientific Library，随机数分布
#include <gsl/gsl_linalg.h> // GNU Scientific Library，线性代数

#include <direct.h> // 建立标识文件夹使用
#include <io.h> // 建立标识文件夹使用

using namespace std;
typedef long long   int64; // 定义别名
typedef unsigned long long uint64; // 定义别名
#define FMT_INT64   "%lld"	// long long ，C语言中表示整数的最大类型
#define FMT_UINT64   "%llu"	// unsigned long long，C语言中表示大于等于零整数的最大类型
#define FMT_HEX64   "%llx" // long long 16进制
...
//global variables
int nsample, nsnp, ncase, nctrl; // 样例数，snp数，对照数，病例数
...
// FILE类型，是输出文件,共有三个
FILE* single_sig_result, * pairwise_sig_result, * high_order_result;

/*
	1. longint对照数，longint病例数
	2. 不知
*/
...
/*
*	1. 啊啊啊不要用缩写呀，signal阈值？
*	2. 规模因子，有下划线，有驼峰，还有啥也没有
*	3. 最显著交互作用的数量
*	4. 搜索使用的类型
*	5. 线程数， 声明了，但是没有用到，啊这
*	这几个都在inputParameters中可以更改 5/13
* 
*	6. 候选计数
* 
*	7. bonferroni_pari_threshold;
*	8. 不知
*	9. 不知
*  10. 不知
*  11. 不知
...
/*
*	1. 迭代次数，蚂蚁的数量， 每次迭代每只蚂蚁选择的SNPs数量，nLociSet, 在一个上位性相互作用中的SNPs数量,在ACO搜索阶段中最高排名单倍型数量
*	2. 信息素、cdf 11/13
*	3. 蚁群算法的蒸发比例，由蚂蚁存入的信息素的权重，level 13/13
*	4. 被选择的LociSet的数量
*	5. 不知
*	6. 不知
*/
...
// static variable for the precomput bin count of 64 bit string，不知
static unsigned char wordbits[65536];// { bitcounts of ints between 0 and 65535 };

// compute number of 1s in 64 bit string，不知
...
/*
* Seeker中含有蚂蚁对象
* 函数： getAnt(),updatePheromone(), startSearch()
* 
*/
class seeker
{
...
/*
* 是一些数学基础相关的东西，返回个随机数什么的
* bitCount(unit64 i): 不知道什么用
* rnd(int uper): 返回小于uper大于等于0的随机数
* rnd(int low, double uper): 返回一个在low到uper之间的随机数？
* cdf2loci(),通过递归把一个叫做cdf的东西转为一个叫loci的东西
* comb_num(),等会再读代码
* comb_increase() ,数学相关
*/
int bitCount(uint64 i)
...
/*
* 数学调用相关
* cmp() compare, vector<double>类型中的index为3的数据a是否大于b
*/
...
/*
* getDataSize(),获取数据大小，并通过指针把sample和snp的个数返回出去
* getCaseControlSize(), 获取病例数和对照数，并把case和control的个数通过指针返回出去
* readData(),加载数据，并放到genocrtl和genoCase数组中
* 
*/
void GetDataSize(const char* filename, int* nsample, int* nsnp)
{
...
* 较复杂的数学调用函数
* chi_square()
* calculateMarginalDistr()
* calsingledev()
* caledev()
* calcsingledev_exclmiss()
*/
double chi_square(int* selectedSNPSet, int m)
{
...
// 常量
const char* inputDataPath = "G:/SNPalgorithm/HiSeeker/inputData/test_data.txt";
const string prefixPath = "D:/SNPPlatfromData/";
const string sig = "single-sig.txt";
const string pairs = "pair-sig.txt";
...
// 将文件路径由string转为char*;
const char* singleSig;
const char* pairSig;
const char* highOrderSig;


//const char* inputDataPathAndName = stringInputDataPathAndName.data();
...
int main(int argc, char* argv[])
{
	...
	//countOfcandidate = 0;
	sig_threshold = 0.05;
	scalefactor = 10000; // 两位点组合个数,太少的话蚁群算法会报错，至少要一个蚂蚁有一个两位点组合
	...
	*  默认的参数不需要改，当做声明和赋值
	*
	* 现在传入前端的参数，共13个参数, 前12个参数算法相关，最后一个参数为文件名。
	* 路径使用绝对路径，使用的时候直接在用户磁盘上新建文件夹。Mac或其它系统就换个版本
	* 顺序按照param.txt来
	* 加一个判断，看是否传入参数，传入就使用参数，不传就使用默认值
	*/

	if (argc > 1) {
		sig_threshold = strtod(argv[1], NULL);
		scalefactor = atoi(argv[2]);
		rou = strtod(argv[3], NULL);
		level = strtod(argv[4], NULL);
		alpha = strtod(argv[5], NULL);
		nAntCount = atoi(argv[6]);
		IterCount = atoi(argv[7]);
		kLociSet = atoi(argv[8]);
		kEpiModel = atoi(argv[9]);
		TopKModel = atoi(argv[10]);
		topK = atoi(argv[11]);
		typeOfSearch = atoi(argv[12]);
		...
		// 完整返回文件路径
		stringSig = prefixPath + stringQueryId + "/resultData/" + sig;
		stringPair = prefixPath + stringQueryId + "/resultData/" + pairs;
		...
		singleSig = stringSig.data();
		pairSig = stringPair.data();
		highOrderSig = stringHighOrder.data();
		cout << "一阶：" << singleSig << endl;
		cout << "二阶：" << pairSig << endl;
	...
	// 写一个简单的判断，不至于报错,穷举时节省时间
	if (typeOfSearch == 0)
		scalefactor = 10;

	/*
	* 使用sprintf需要提前声明数组，越大越好
	* filename为读进来的data文件，动态分配空间
	* MAFs，Samples，LDs，maf,samp,ld,h暂时未知什么用，大概是常量
	*/
	...
	/*
	* 前面两个for循环是在干嘛
	* sprintf是将第二个参数中的文件中的内容打印到result1name中
	* fopen返回FILE类型指针，‘w’表示write,可以利用FILE指针开始写了
	* 后面的fprintf表示写在这三个文件中，把标题写上
	* FILE类型指针filename存储数据文件，用sprintf写到filename中
	* 
	*/
	for (int ma = 0; ma < 1; ma++) {
		...
		for (int fileno = 0; fileno < 1; fileno++) {
			...
			sprintf(result1name, singleSig);
			sprintf(result2name, pairSig);
			sprintf(result3name, highOrderSig);
			...
			sprintf(filename, inputDataPath);
