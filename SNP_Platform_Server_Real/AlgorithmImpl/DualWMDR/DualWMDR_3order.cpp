...
void CV() {
	...
	int p, q;
	//int *tmp = (int*)malloc((nsnp+1)*sizeof(int));
	int tmp;
...
int Partition_mdr(double a[], double a0[], double a1[], double a2[], int low, int high)
{
	double x = a[high];
	int i = low - 1;
	for (int j = low; j < high; j++)
...
void QuickSort_mdr(double a[], double a0[], double a1[], double a2[], int low, int high)
{
	if (low < high)
	{
		int q = Partition_mdr(a, a0, a1, a2, low, high);
		QuickSort_mdr(a, a0, a1, a2, low, q - 1);
		QuickSort_mdr(a, a0, a1, a2, q + 1, high);
	}
}
...
void MDR_interactions() {
	...
	uint64 mask1 = 0x0000000000000001;
	int* snps = new int[2];
	double r_train[3][3][3], r_test[3][3][3];
	int flag_train[3][3][3], flag_test[3][3][3];
	double w[3][3][3];
	double TP_w, FP_w, TN_w, FN_w;
	int o = comb_num(candidate.size(), 2) * nsnp;
	double** per;
	...
	uint64* z_c00, * z_c01, * z_c02, * z_c03, * z_c10, * z_c11, * z_c12, * z_c13;
	int casefreq_train[4][4][4], ctrlfreq_train[4][4][4], casefreq_test[4][4][4], ctrlfreq_test[4][4][4];
	int m, n, t, k, h;
	double sum;
		...
		for (j1 = 0; j1 < candidate.size() - 1; j1++){
			for (j2 = j1 + 1; j2 < candidate.size(); j2++){
				for (j3 = 0; j3 < nsnp; j3++){
					...
					if (j3 != snps[0] && j3 != snps[1]){
						snps[2] = j3;
		*/

		for (j1 = 0; j1 < candidate.size() - 2; j1++) {
			for (j2 = j1 + 1; j2 < candidate.size() - 1; j2++) {
				for (j3 = j2 + 1; j3 < candidate.size(); j3++) {
					snps[0] = (int)candidate[j1];
					snps[1] = (int)candidate[j2];
					snps[2] = (int)candidate[j3];
					...
					z_c00 = genocase_train_c0[snps[2]]; z_c01 = genocase_train_c1[snps[2]];
					z_c02 = genocase_train_c2[snps[2]]; z_c03 = genocase_train_c3[snps[2]];
					z_c10 = genoctrl_train_c0[snps[2]]; z_c11 = genoctrl_train_c1[snps[2]];
					z_c12 = genoctrl_train_c2[snps[2]]; z_c13 = genoctrl_train_c3[snps[2]];

					for (m = 0; m < 3; m++)
						for (n = 0; n < 3; n++)
							for (t = 0; t < 3; t++) {
								casefreq_train[m][n][t] = 0;
								ctrlfreq_train[m][n][t] = 0;
							}

					for (h = 0; h < nlongintcase; h++) {
						x = x_c00[h] & y_c00[h] & z_c00[h]; casefreq_train[0][0][0] += popcount(x);
						x = x_c00[h] & y_c00[h] & z_c01[h]; casefreq_train[0][0][1] += popcount(x);
						x = x_c00[h] & y_c00[h] & z_c02[h]; casefreq_train[0][0][2] += popcount(x);
						x = x_c00[h] & y_c01[h] & z_c00[h]; casefreq_train[0][1][0] += popcount(x);
						x = x_c00[h] & y_c01[h] & z_c01[h]; casefreq_train[0][1][1] += popcount(x);
						x = x_c00[h] & y_c01[h] & z_c02[h]; casefreq_train[0][1][2] += popcount(x);
						x = x_c00[h] & y_c02[h] & z_c00[h]; casefreq_train[0][2][0] += popcount(x);
						x = x_c00[h] & y_c02[h] & z_c01[h]; casefreq_train[0][2][1] += popcount(x);
						x = x_c00[h] & y_c02[h] & z_c02[h]; casefreq_train[0][2][2] += popcount(x);

						x = x_c01[h] & y_c00[h] & z_c00[h]; casefreq_train[1][0][0] += popcount(x);
						x = x_c01[h] & y_c00[h] & z_c01[h]; casefreq_train[1][0][1] += popcount(x);
						x = x_c01[h] & y_c00[h] & z_c02[h]; casefreq_train[1][0][2] += popcount(x);
						x = x_c01[h] & y_c01[h] & z_c00[h]; casefreq_train[1][1][0] += popcount(x);
						x = x_c01[h] & y_c01[h] & z_c01[h]; casefreq_train[1][1][1] += popcount(x);
						x = x_c01[h] & y_c01[h] & z_c02[h]; casefreq_train[1][1][2] += popcount(x);
						x = x_c01[h] & y_c02[h] & z_c00[h]; casefreq_train[1][2][0] += popcount(x);
						x = x_c01[h] & y_c02[h] & z_c01[h]; casefreq_train[1][2][1] += popcount(x);
						x = x_c01[h] & y_c02[h] & z_c02[h]; casefreq_train[1][2][2] += popcount(x);

						x = x_c02[h] & y_c00[h] & z_c00[h]; casefreq_train[2][0][0] += popcount(x);
						x = x_c02[h] & y_c00[h] & z_c01[h]; casefreq_train[2][0][1] += popcount(x);
						x = x_c02[h] & y_c00[h] & z_c02[h]; casefreq_train[2][0][2] += popcount(x);
						x = x_c02[h] & y_c01[h] & z_c00[h]; casefreq_train[2][1][0] += popcount(x);
						x = x_c02[h] & y_c01[h] & z_c01[h]; casefreq_train[2][1][1] += popcount(x);
						x = x_c02[h] & y_c01[h] & z_c02[h]; casefreq_train[2][1][2] += popcount(x);
						x = x_c02[h] & y_c02[h] & z_c00[h]; casefreq_train[2][2][0] += popcount(x);
						x = x_c02[h] & y_c02[h] & z_c01[h]; casefreq_train[2][2][1] += popcount(x);
						x = x_c02[h] & y_c02[h] & z_c02[h]; casefreq_train[2][2][2] += popcount(x);
					}

					for (h = 0; h < nlongintctrl; h++) {
						x = x_c10[h] & y_c10[h] & z_c10[h]; ctrlfreq_train[0][0][0] += popcount(x);
						x = x_c10[h] & y_c10[h] & z_c11[h]; ctrlfreq_train[0][0][1] += popcount(x);
						x = x_c10[h] & y_c10[h] & z_c12[h]; ctrlfreq_train[0][0][2] += popcount(x);
						x = x_c10[h] & y_c11[h] & z_c10[h]; ctrlfreq_train[0][1][0] += popcount(x);
						x = x_c10[h] & y_c11[h] & z_c11[h]; ctrlfreq_train[0][1][1] += popcount(x);
						x = x_c10[h] & y_c11[h] & z_c12[h]; ctrlfreq_train[0][1][2] += popcount(x);
						x = x_c10[h] & y_c12[h] & z_c10[h]; ctrlfreq_train[0][2][0] += popcount(x);
						x = x_c10[h] & y_c12[h] & z_c11[h]; ctrlfreq_train[0][2][1] += popcount(x);
						x = x_c10[h] & y_c12[h] & z_c12[h]; ctrlfreq_train[0][2][2] += popcount(x);

						x = x_c11[h] & y_c10[h] & z_c10[h]; ctrlfreq_train[1][0][0] += popcount(x);
						x = x_c11[h] & y_c10[h] & z_c11[h]; ctrlfreq_train[1][0][1] += popcount(x);
						x = x_c11[h] & y_c10[h] & z_c12[h]; ctrlfreq_train[1][0][2] += popcount(x);
						x = x_c11[h] & y_c11[h] & z_c10[h]; ctrlfreq_train[1][1][0] += popcount(x);
						x = x_c11[h] & y_c11[h] & z_c11[h]; ctrlfreq_train[1][1][1] += popcount(x);
						x = x_c11[h] & y_c11[h] & z_c12[h]; ctrlfreq_train[1][1][2] += popcount(x);
						x = x_c11[h] & y_c12[h] & z_c10[h]; ctrlfreq_train[1][2][0] += popcount(x);
						x = x_c11[h] & y_c12[h] & z_c11[h]; ctrlfreq_train[1][2][1] += popcount(x);
						x = x_c11[h] & y_c12[h] & z_c12[h]; ctrlfreq_train[1][2][2] += popcount(x);

						x = x_c12[h] & y_c10[h] & z_c10[h]; ctrlfreq_train[2][0][0] += popcount(x);
						x = x_c12[h] & y_c10[h] & z_c11[h]; ctrlfreq_train[2][0][1] += popcount(x);
						x = x_c12[h] & y_c10[h] & z_c12[h]; ctrlfreq_train[2][0][2] += popcount(x);
						x = x_c12[h] & y_c11[h] & z_c10[h]; ctrlfreq_train[2][1][0] += popcount(x);
						x = x_c12[h] & y_c11[h] & z_c11[h]; ctrlfreq_train[2][1][1] += popcount(x);
						x = x_c12[h] & y_c11[h] & z_c12[h]; ctrlfreq_train[2][1][2] += popcount(x);
						x = x_c12[h] & y_c12[h] & z_c10[h]; ctrlfreq_train[2][2][0] += popcount(x);
						x = x_c12[h] & y_c12[h] & z_c11[h]; ctrlfreq_train[2][2][1] += popcount(x);
						x = x_c12[h] & y_c12[h] & z_c12[h]; ctrlfreq_train[2][2][2] += popcount(x);
					}

					for (m = 0; m < 3; m++) {
						for (n = 0; n < 3; n++) {
							for (t = 0; t < 3; t++) {
								if (casefreq_train[m][n][t] == 0 && ctrlfreq_train[m][n][t] != 0) {
									r_train[m][n][t] = (double(casefreq_train[m][n][t]) + constant) / double(ctrlfreq_train[m][n][t]) * ((double)nctrl / (double)ncase);
									if (r_train[m][n][t] == mdr_threshold) {
										r_train[m][n][t] = r_train[m][n][t] + constant;
										flag_train[m][n][t] = 1;
									}
									else if (r_train[m][n][t] > mdr_threshold) flag_train[m][n][t] = 1;
									else flag_train[m][n][t] = 0;
								}
								else if (casefreq_train[m][n][t] != 0 && ctrlfreq_train[m][n][t] == 0) {
									r_train[m][n][t] = double(casefreq_train[m][n][t]) / (double(ctrlfreq_train[m][n][t]) + constant) * ((double)nctrl / (double)ncase);
									if (r_train[m][n][t] == mdr_threshold) {
										r_train[m][n][t] = r_train[m][n][t] + constant;
										flag_train[m][n][t] = 1;
									}
									...
									r_train[m][n][t] = double(casefreq_train[m][n][t]) / double(ctrlfreq_train[m][n][t]) * ((double)nctrl / (double)ncase);
									if (r_train[m][n][t] == mdr_threshold) {
										r_train[m][n][t] = r_train[m][n][t] + constant;
										flag_train[m][n][t] = 1;
									}
					...
					z_c00 = genocase_test_c0[snps[2]]; z_c01 = genocase_test_c1[snps[2]];
					z_c02 = genocase_test_c2[snps[2]]; z_c03 = genocase_test_c3[snps[2]];
					z_c10 = genoctrl_test_c0[snps[2]]; z_c11 = genoctrl_test_c1[snps[2]];
					z_c12 = genoctrl_test_c2[snps[2]]; z_c13 = genoctrl_test_c3[snps[2]];

					for (m = 0; m < 4; m++)
						for (n = 0; n < 4; n++)
							for (t = 0; t < 3; t++) {
								casefreq_test[m][n][t] = 0;
								ctrlfreq_test[m][n][t] = 0;
							}

					for (h = 0; h < nlongintcase; h++) {
						x = x_c00[h] & y_c00[h] & z_c00[h]; casefreq_test[0][0][0] += popcount(x);
						x = x_c00[h] & y_c00[h] & z_c01[h]; casefreq_test[0][0][1] += popcount(x);
						x = x_c00[h] & y_c00[h] & z_c02[h]; casefreq_test[0][0][2] += popcount(x);
						x = x_c00[h] & y_c01[h] & z_c00[h]; casefreq_test[0][1][0] += popcount(x);
						x = x_c00[h] & y_c01[h] & z_c01[h]; casefreq_test[0][1][1] += popcount(x);
						x = x_c00[h] & y_c01[h] & z_c02[h]; casefreq_test[0][1][2] += popcount(x);
						x = x_c00[h] & y_c02[h] & z_c00[h]; casefreq_test[0][2][0] += popcount(x);
						x = x_c00[h] & y_c02[h] & z_c01[h]; casefreq_test[0][2][1] += popcount(x);
						x = x_c00[h] & y_c02[h] & z_c02[h]; casefreq_test[0][2][2] += popcount(x);

						x = x_c01[h] & y_c00[h] & z_c00[h]; casefreq_test[1][0][0] += popcount(x);
						x = x_c01[h] & y_c00[h] & z_c01[h]; casefreq_test[1][0][1] += popcount(x);
						x = x_c01[h] & y_c00[h] & z_c02[h]; casefreq_test[1][0][2] += popcount(x);
						x = x_c01[h] & y_c01[h] & z_c00[h]; casefreq_test[1][1][0] += popcount(x);
						x = x_c01[h] & y_c01[h] & z_c01[h]; casefreq_test[1][1][1] += popcount(x);
						x = x_c01[h] & y_c01[h] & z_c02[h]; casefreq_test[1][1][2] += popcount(x);
						x = x_c01[h] & y_c02[h] & z_c00[h]; casefreq_test[1][2][0] += popcount(x);
						x = x_c01[h] & y_c02[h] & z_c01[h]; casefreq_test[1][2][1] += popcount(x);
						x = x_c01[h] & y_c02[h] & z_c02[h]; casefreq_test[1][2][2] += popcount(x);

						x = x_c02[h] & y_c00[h] & z_c00[h]; casefreq_test[2][0][0] += popcount(x);
						x = x_c02[h] & y_c00[h] & z_c01[h]; casefreq_test[2][0][1] += popcount(x);
						x = x_c02[h] & y_c00[h] & z_c02[h]; casefreq_test[2][0][2] += popcount(x);
						x = x_c02[h] & y_c01[h] & z_c00[h]; casefreq_test[2][1][0] += popcount(x);
						x = x_c02[h] & y_c01[h] & z_c01[h]; casefreq_test[2][1][1] += popcount(x);
						x = x_c02[h] & y_c01[h] & z_c02[h]; casefreq_test[2][1][2] += popcount(x);
						x = x_c02[h] & y_c02[h] & z_c00[h]; casefreq_test[2][2][0] += popcount(x);
						x = x_c02[h] & y_c02[h] & z_c01[h]; casefreq_test[2][2][1] += popcount(x);
						x = x_c02[h] & y_c02[h] & z_c02[h]; casefreq_test[2][2][2] += popcount(x);
					}

					for (h = 0; h < nlongintctrl; h++) {
						x = x_c10[h] & y_c10[h] & z_c10[h]; ctrlfreq_test[0][0][0] += popcount(x);
						x = x_c10[h] & y_c10[h] & z_c11[h]; ctrlfreq_test[0][0][1] += popcount(x);
						x = x_c10[h] & y_c10[h] & z_c12[h]; ctrlfreq_test[0][0][2] += popcount(x);
						x = x_c10[h] & y_c11[h] & z_c10[h]; ctrlfreq_test[0][1][0] += popcount(x);
						x = x_c10[h] & y_c11[h] & z_c11[h]; ctrlfreq_test[0][1][1] += popcount(x);
						x = x_c10[h] & y_c11[h] & z_c12[h]; ctrlfreq_test[0][1][2] += popcount(x);
						x = x_c10[h] & y_c12[h] & z_c10[h]; ctrlfreq_test[0][2][0] += popcount(x);
						x = x_c10[h] & y_c12[h] & z_c11[h]; ctrlfreq_test[0][2][1] += popcount(x);
						x = x_c10[h] & y_c12[h] & z_c12[h]; ctrlfreq_test[0][2][2] += popcount(x);

						x = x_c11[h] & y_c10[h] & z_c10[h]; ctrlfreq_test[1][0][0] += popcount(x);
						x = x_c11[h] & y_c10[h] & z_c11[h]; ctrlfreq_test[1][0][1] += popcount(x);
						x = x_c11[h] & y_c10[h] & z_c12[h]; ctrlfreq_test[1][0][2] += popcount(x);
						x = x_c11[h] & y_c11[h] & z_c10[h]; ctrlfreq_test[1][1][0] += popcount(x);
						x = x_c11[h] & y_c11[h] & z_c11[h]; ctrlfreq_test[1][1][1] += popcount(x);
						x = x_c11[h] & y_c11[h] & z_c12[h]; ctrlfreq_test[1][1][2] += popcount(x);
						x = x_c11[h] & y_c12[h] & z_c10[h]; ctrlfreq_test[1][2][0] += popcount(x);
						x = x_c11[h] & y_c12[h] & z_c11[h]; ctrlfreq_test[1][2][1] += popcount(x);
						x = x_c11[h] & y_c12[h] & z_c12[h]; ctrlfreq_test[1][2][2] += popcount(x);

						x = x_c12[h] & y_c10[h] & z_c10[h]; ctrlfreq_test[2][0][0] += popcount(x);
						x = x_c12[h] & y_c10[h] & z_c11[h]; ctrlfreq_test[2][0][1] += popcount(x);
						x = x_c12[h] & y_c10[h] & z_c12[h]; ctrlfreq_test[2][0][2] += popcount(x);
						x = x_c12[h] & y_c11[h] & z_c10[h]; ctrlfreq_test[2][1][0] += popcount(x);
						x = x_c12[h] & y_c11[h] & z_c11[h]; ctrlfreq_test[2][1][1] += popcount(x);
						x = x_c12[h] & y_c11[h] & z_c12[h]; ctrlfreq_test[2][1][2] += popcount(x);
						x = x_c12[h] & y_c12[h] & z_c10[h]; ctrlfreq_test[2][2][0] += popcount(x);
						x = x_c12[h] & y_c12[h] & z_c11[h]; ctrlfreq_test[2][2][1] += popcount(x);
						x = x_c12[h] & y_c12[h] & z_c12[h]; ctrlfreq_test[2][2][2] += popcount(x);
					}

					for (m = 0; m < 3; m++) {
						for (n = 0; n < 3; n++) {
							for (t = 0; t < 3; t++) {
								r_test[m][n][t] = double(casefreq_test[m][n][t]) / double(ctrlfreq_test[m][n][t]) * ((double)nctrl / (double)ncase);
								if (casefreq_test[m][n][t] == 0 && ctrlfreq_test[m][n][t] != 0) {
									r_test[m][n][t] = (double(casefreq_test[m][n][t]) + constant) / double(ctrlfreq_test[m][n][t]) * ((double)nctrl / (double)ncase);
									if (r_test[m][n][t] == mdr_threshold) {
										r_test[m][n][t] = r_test[m][n][t] + constant;
										flag_test[m][n][t] = 1;
									}
									else if (r_test[m][n][t] > mdr_threshold) flag_test[m][n][t] = 1;
									else flag_test[m][n][t] = 0;
								}
								else if (casefreq_test[m][n][t] != 0 && ctrlfreq_test[m][n][t] == 0) {
									r_test[m][n][t] = double(casefreq_test[m][n][t]) / (double(ctrlfreq_test[m][n][t]) + constant) * ((double)nctrl / (double)ncase);
									if (r_test[m][n][t] == mdr_threshold) {
										r_test[m][n][t] = r_test[m][n][t] + constant;
										flag_test[m][n][t] = 1;
									}
					...
					for (m = 0; m < 3; m++) {
						for (n = 0; n < 3; n++) {
							for (t = 0; t < 3; t++) {
								if (flag_train[m][n][t] != -1 && flag_test[m][n][t] != -1) {
									if (flag_train[m][n][t] == 1 && flag_test[m][n][t] == 1) {

										if (casefreq_train[m][n][t] < casefreq_test[m][n][t]) {
											w[m][n][t] = pow(fabs(log10f(r_train[m][n][t])), alpha);
										}
										else {
											w[m][n][t] = pow(fabs(log10f(r_test[m][n][t])), alpha);
										}
										TP_w += w[m][n][t] * double(casefreq_train[m][n][t] + casefreq_test[m][n][t]);

										if (ctrlfreq_train[m][n][t] < ctrlfreq_test[m][n][t]) {
											w[m][n][t] = pow(fabs(log10f(r_train[m][n][t])), alpha);
										}
										else {
											w[m][n][t] = pow(fabs(log10f(r_test[m][n][t])), alpha);
										}
										FP_w += w[m][n][t] * double(ctrlfreq_train[m][n][t] + ctrlfreq_test[m][n][t]);
									}

									else if (flag_train[m][n][t] == 0 && flag_test[m][n][t] == 0) {
										if (casefreq_train[m][n][t] < casefreq_test[m][n][t]) {
											w[m][n][t] = pow(fabs(log10f(r_train[m][n][t])), alpha);
										}
										else {
										...
										FN_w += w[m][n][t] * double(casefreq_train[m][n][t] + casefreq_test[m][n][t]);
										...
										TN_w += w[m][n][t] * double(ctrlfreq_train[m][n][t] + ctrlfreq_test[m][n][t]);
									}
								}
							}
						}
					...
					interactionInfos[0][count_inter] = snps[0];
					interactionInfos[1][count_inter] = snps[1];
					interactionInfos[2][count_inter] = snps[2];
				}
				//}
			}
		}
	...
	for (i = 0; i < count_inter + 1; i++) {
		...
		interactionInfos[3][i] = sum / double(fold);
	}

	QuickSort_mdr(interactionInfos[3], interactionInfos[0], interactionInfos[1], interactionInfos[2], 0, count_inter);

	for (k = 0; k < topK; k++) {
		fprintf(high_order_result, "%d\t%d\t%d\t%f\n", int(interactionInfos[0][k]), int(interactionInfos[1][k]), int(interactionInfos[2][k]), interactionInfos[3][k]);
	}
...
int main(int argc, char* argv[]) {
	...
	if (argc > 1) {
		...
		inputfile = "G:/SNPalgorithm/DualWMDR_3order/testInputData/test_data_2loci.txt";
		outputfile = "G:/SNPalgorithm/DualWMDR_3order/testResData/resData.txt";
	}

	//******************修改部分结束*********************

	char resultname[10000] = "";
	char* filename = (char*)malloc(10000 * sizeof(char));

	sprintf(resultname, outputfile);
	high_order_result = fopen(resultname, "w");
	fprintf(high_order_result, "SNP1\tSNP2\tSNP3\taverage prediction error rates\n");
	...
	interactionInfos = (double**)calloc(4, sizeof(double));
	for (i = 0; i < 4; i++)
		interactionInfos[i] = (double*)calloc(comb_num(candidate.size(), 2) * nsnp, sizeof(double));
