...
public class MACOEDController {
     ...
     * 并强制返回轮询结果为true
     */
    @GetMapping("/MACOEDForceStop")
    public String macoedForceStop(@RequestParam Map<String, String> params) throws FileNotFoundException {

        // 强制终止所有进程
        MACOEDImpl.destroyOb();
        // 保存目前计算的结果，打包成压缩包
        String queryId = macoedp.getQueryId();
        String goalPath = MyConst.TEM_DATA_PATH + queryId + "/resultData";
        ZipUtils.dirToZip(goalPath);
        // 返回轮询结果为true
        finished = "true";
